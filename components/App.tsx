...
const App = () => {
  ...
  const fetchInvoicesCloud = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Recovery logic for sequences based on fetched data
        ...
        const mapped: Invoice[] = data.map(f => {
          ...
          return {
            ...
            isCertified: !!f.hash, // FIX: derived from hash presence to avoid auto-certification
            hash: f.hash || '',
        ...
        // Apply recovered sequences to the active series
  ...
  const handleSaveInvoice = async (inv: Invoice, sId: string, action?: string) => {
      ...
      try {
        ...
        const { error } = await supabase.from('faturas').upsert({
          ...
          hash: finalInv.hash // FIX: Perist hash to keep certification state
        });
  ...
  const handleDeleteInvoice = async (id: string) => {
      // REGRA AGT: Apenas rascunhos podem ser apagados se o sistema permitir. 
      // Faturas certificadas devem ser anuladas.
      const inv = invoices.find(i => i.id === id);
      if (inv?.isCertified) {
          alert("Documentos certificados não podem ser apagados por imposição legal (AGT). Utilize a função de anulação.");
          return;
      }
  ...
  const handleLiquidate = async (invoice: Invoice, amount: number, method: PaymentMethod, registerId: string, dateValue: string, docDate: string) => {
      ...
      const number = `RC ${docSeries.year}/${nextSeq}`;
      ...
      try {
        ...
        // Guardar o Recibo (Documento Fiscal)
        ...
        // Atualizar o estado da fatura original para 'Pago' ou 'Parcial'
